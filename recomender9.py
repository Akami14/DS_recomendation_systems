...
class MainRecommender:
    ...
    def __init__(self, data, weighting=None, false_id=999999):
        ...
        self.item_recommender = self._fit_item_recommender(self.user_item_matrix)
        #self.bm25_recommender = self._fit_bm25_recomender(self.user_item_matrix)
    ...
    @staticmethod
    def _fit_item_recommender(user_item_matrix, K=320):
        """Обучает модель, которая рекомендует товары, среди товаров, купленных юзером"""

        item_recommender = ItemItemRecommender(K=K, num_threads=4)
        item_recommender.fit(user_item_matrix)

        return item_recommender

   #@staticmethod
   #def _fit_bm25_recomender(user_item_matrix, K=260):
   #    bm25_recommender = BM25Recommender(K=K, num_threads=4)  # K - кол-во билжайших соседей

   #    bm25_recommender.fit(user_item_matrix)
   #    return bm25_recommender
    ...
    def _get_recommendations(self, user, model, N=200):
    ...
    def get_item_recommendations(self, user, N=200):
        """Рекомендуем товары среди тех, которые юзер уже купил"""

        self._update_dict(user_id=user)
        return self._get_recommendations(user, model=self.item_recommender, N=N)

   #def get_bm25_recommendations(self, user, N=200):
   #    """Рекомендуем товары среди тех, которые юзер уже купил"""
